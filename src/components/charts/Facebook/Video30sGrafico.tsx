...
interface Video30sGraficoProps {
  theme: string;
  page_id?: string;
}

const Video30sGrafico: React.FC<Video30sGraficoProps> = ({ theme, page_id }) => {
  ...
  const ReproduccionesPorDia = metricsData?.Crecimiento?.['Reproducciones 30s por Día']?.map((item: { date: string; value: number }) => ({
  ...
  return (
    <div
      ...
      >
        ...
        <div className="flex p-4 mt-3 x-full md:w-4/12">
                  ...
                  <h3 className="text-xl font-semibold flex items-center text-center">
                    <IoMdPlay className="text-2xl mr-2" />
                    <strong className="font-bold">Reproducciones de 30 seg.</strong>
                  </h3>
        
                    <div>
                      <p className="flex justify-between">
                        <strong className='text-gray-700'>Total Alcance :</strong> 
                        <strong className='text-xl'>{ReproduccionesPorDia.reduce((sum, item) => sum + item.reach, 0)}</strong>
                      </p>
                      <p className='flex justify-between'>
                        <strong className='text-gray-700'>Promedio Diario :</strong> 
                        <strong className='text-xl'>{(ReproduccionesPorDia.reduce((sum, item) => sum + item.reach, 0) / ReproduccionesPorDia.length).toFixed(0)}</strong>
                      </p>
                      <p><strong className='text-gray-700'>Cantidad por Día :</strong></p>
                        <div className="mt-2 max-h-52 overflow-y-auto p-4 rounded-3xl shadow-inner border border-gray-200">
                          ...
                          <ul>
                            {ReproduccionesPorDia.map((item, index) => {
...
export default Video30sGrafico;