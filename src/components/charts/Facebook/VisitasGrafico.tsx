...
const VisitasGrafico: React.FC<VisitasGraficoProps> = ({ theme, page_id }) => {
  ...
  // Uso de React Query para obtener los datos con el rango de fechas
  ...
  const visitasPorDia = metricsData?.Crecimiento?.['Visitas a la Página por Día']?.map((item: { date: string; value: number }) => ({
      ...
      >
        ...
        <div className="flex p-4 mt-3 w-full md:w-4/12">
          ...
          <h3 className="text-xl font-semibold flex items-center text-center">
            ...
            <strong className="font-bold">Desglose de Visitas</strong>
          </h3>
        
             <div>
              <p className="flex justify-between">
                 <strong className='text-gray-700'>Total Visitas :</strong> 
                <strong className='text-xl'>{visitasPorDia.reduce((sum, item) => sum + item.visits, 0)}</strong>
               </p>
               <p className='flex justify-between'>
                <strong className='text-gray-700'>Promedio Diario :</strong> 
                 <strong className='text-xl'>{(visitasPorDia.reduce((sum, item) => sum + item.visits, 0) / visitasPorDia.length).toFixed(0)}</strong>
              </p>
