...
interface AlcanceGraficoIgProps {
  theme: string;
  page_id?: string;
}

const AlcanceGraficoIg: React.FC<AlcanceGraficoIgProps> = ({ theme, page_id }) => {
  const chartRef = useRef<HTMLDivElement>(null); // Ref para capturar el gráfico
  ...
  const alcancePorDia = metricsData?.Crecimiento['Alcance por Día']?.map((item) => ({
...
export default AlcanceGraficoIg;
