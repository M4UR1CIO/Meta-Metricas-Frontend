...
const GenderGraphIg: React.FC<GenderGraphProps> = ({ page_id }) => {
  ...
  const { data: genderData, isLoading: isLoadingDemographics, isError: isErrorDemographics, error: errorDemographics } =
    useQuery({
      queryKey: ['instagramGender', page_id],
      queryFn: async () => fetchDemographics(page_id, 'gender'),
      enabled: followersCount !== null && followersCount >= 100,
  
    });

  const genderChartData = Object.entries(genderData?.demographics || {}).map(([key, value]) => {
    const name =
      key === 'M' ? 'Hombre' : key === 'F' ? 'Mujer' : key === 'U' ? 'Desconocido' : key;
    return { name, value };
  });
