  ...
  const CityGraphIg: React.FC<CityGraphProps> = ({ page_id }) => {
    ...
    const { data: cityData, isLoading: isLoadingDemographics, isError: isErrorDemographics, error: errorDemographics } =
      useQuery({
        queryKey: ['instagramCity', page_id],
        queryFn: async () => fetchDemographics(page_id, 'city'),
        enabled: followersCount !== null && followersCount >= 100,
      });

    const processedCityData = Object.entries(cityData?.demographics || {})
    ...
    const cityChartData = [...processedCityData.top, processedCityData.others];

    const COLORS = ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#B7245C', '#FFD700', '#CCCCCC'];
