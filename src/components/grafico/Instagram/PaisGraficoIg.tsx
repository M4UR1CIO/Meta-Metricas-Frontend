...
import { DemographicsData } from '../../../types/metricsTypes';
...
interface CountryGraphIgProps {
  page_id: string;
}

const CountryGraphIg: React.FC<CountryGraphIgProps> = ({ page_id }) => {
  ...
  const { data: countryData, isLoading: isLoadingDemographics, isError: isErrorDemographics, error: errorDemographics } =
    useQuery({
      queryKey: ['instagramCountry', page_id],
      queryFn: async () => fetchDemographics(page_id, 'country'),
      enabled: followersCount !== null && followersCount >= 100,
    });

  const processedCountryData = Object.entries(countryData?.demographics || {})
    .map(([key, value]) => ({
      name: countries.getName(key, 'es') || key,
      value,
    }))
  ...
  const COLORS = ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', '#B7245C', '#FFD700', '#102B3F', '#A06CD5', '#F85E00', '#CCCCCC'];
